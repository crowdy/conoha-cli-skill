...
4. **`conoha app` サブコマンドでは `--app-name` を必ず指定する** — 省略するとアプリ名の入力プロンプトが発生する。`--app-name` は **DNS-1123 ラベル形式** (小文字英数字とハイフンのみ、アンダースコア不可、1–63 文字) で指定すること。アンダースコアを含む名前は `app init/deploy` と `app destroy` の間でパス解決が食い違い、サーバー側の作業ディレクトリがクリーンアップされずに残ることがある (`/opt/conoha/<name>` の残留)
